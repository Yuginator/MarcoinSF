...
const generateLayout = (): LayoutItem[] => {
  return MEDIA_DATA.map((item, i) => {
    ...
    // Randomize border style: 0 (light), 1 (medium), 2 (heavy)
    const rand = Math.random();
    let borderIntensity = 1;
    if (rand < 0.3) borderIntensity = 0;
    else if (rand > 0.7) borderIntensity = 2;
    ...
    return {
      ...
      x: (i % 2 === 0 ? -1 : 1) * CONFIG.X_OFFSET,
      y: (Math.random() - 0.5) * 0.5,
      rotationY: (i % 2 === 0 ? 0.12 : -0.12) + (Math.random() - 0.5) * 0.05,
      rotationZ: (Math.random() - 0.5) * 0.05,
      borderIntensity,
    };
  });
};
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    scene.fog = new THREE.Fog(CONFIG.FOG_COLOR, 10, 60);
    ...
    layout.forEach((item) => {
      ...
      const contentMesh = new THREE.Mesh(initialGeometry.clone(), new THREE.MeshBasicMaterial({ transparent: true, opacity: 0 }));
      contentMesh.position.z = 0.1; 
      contentMesh.position.y = 0.15;
      
      const bgMesh = new THREE.Mesh(initialGeometry.clone(), cardMaterial.clone());
      bgMesh.position.z = 0;
      
      const initialFrameGeo = createSketchyBoxGeometry(3.5, 2.7, item.borderIntensity);
      ...
      if (item.type === 'video') {
        ...
        video.onloadedmetadata = () => {
             updateGeometry(video.videoWidth, video.videoHeight);
        };
        ...
        textureLoader.load(preview, (texture) => {
            texture.colorSpace = THREE.SRGBColorSpace;
            texture.wrapS = THREE.ClampToEdgeWrapping;
            texture.wrapT = THREE.ClampToEdgeWrapping;
            ...
            contentMesh.material.needsUpdate = true;
            if (texture.image) {
                updateGeometry(texture.image.width, texture.image.height);
            }
        });
      } else {
    ...
    const scrollHeight = totalZDist * 250 + window.innerHeight;
    ...
    const animate = () => {
      ...
      const distToFinale = Math.abs(camera.position.z - lastItemZ);
      const effectiveDist = Math.max(0, distToFinale - CONFIG.BGM_SILENCE_RADIUS);
      let targetBgmVol = 1.0;
      if (effectiveDist < CONFIG.BGM_FADE_ZONE) {
          targetBgmVol = Math.max(0, Math.min(1, effectiveDist / CONFIG.BGM_FADE_ZONE));
      }
      bgmVolumeRef.current += (targetBgmVol - bgmVolumeRef.current) * 0.05;
      updateBgmVolume(bgmVolumeRef.current);

      meshesRef.current.forEach(mesh => {
        const group = mesh.parent!;
        const dist = Math.abs(camera.position.z - group.position.z);
        const itemId = group.userData.id;
        ...
        let alpha = 0;
        if (dist <= CONFIG.FADE_START) alpha = 1;
        else if (dist >= CONFIG.FADE_END) alpha = 0;
        else alpha = 1 - (dist - CONFIG.FADE_START) / (CONFIG.FADE_END - CONFIG.FADE_START);
    ...
    <>
      ...
      <div className={`fixed top-6 right-6 z-40 flex flex-col items-end gap-3 transition-opacity duration-500 ${hasEntered ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
        ...
        <div className="relative flex flex-col items-end">
            {/* @ts-ignore */}
            <wired-icon-button class="bg-white text-gray-800" onClick={() => togglePanel('audio')}>
                <IconMusic />
            {/* @ts-ignore */}
            </wired-icon-button>
        ...
        <div className="relative flex flex-col items-end">
             {/* @ts-ignore */}
            <wired-icon-button class="bg-white text-gray-800" onClick={() => togglePanel('general')}>
                <IconSettings />
            {/* @ts-ignore */}
            </wired-icon-button>
        ...
        <div className="relative flex flex-col items-end">
             {/* @ts-ignore */}
            <wired-icon-button class="bg-white text-gray-800" onClick={() => togglePanel('credit')}>
                <IconInfo />
             {/* @ts-ignore */}
            </wired-icon-button>
